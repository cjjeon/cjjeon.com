...
const RatingStars: React.FC<RatingStarsProps> = ({rating}) => {
    ...
    return <div className={"flex gap-2"}>{stars.map((star, index) => {
...
const SkillCards: React.FC<SkillCardsProp> = ({
    ...
    return (
        ...
        >
            <Slide className={'relative z-10'} direction={"down"} isSlide={!isSlide} height={childHeight}>
                <div
                    className={
                        "bg-indigo-300 dark:bg-indigo-500 rounded flex justify-center items-center flex-col p-5"
                    }
                >
            ...
            <Slide className={'relative z-0'} direction={"up"} isSlide={!isSlide} height={childHeight}>
                <div
                    ref={childRef}
                    className={
                        "flex flex-wrap justify-center items-center gap-2 bg-indigo-500 dark:bg-indigo-300 rounded p-3"
                    }
                >
                    {tags.map((tag, index) => {
                        return (
                            <div className={"bg-indigo-300 dark:bg-indigo-500 rounded-full py-1 px-2 text-xs"}
                                 key={index}>
                                {tag}
                            </div>
                        );
